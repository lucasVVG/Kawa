...
and access = Private | Public | Protected
...
type method_def = {
    ...
    locals: ((string* expr option) * typ) list;
    return: typ;
  }

type metatt = 
| Met of method_def 
| Att of string * typ * bool * access
...
type class_def = {
    class_name: string;
    attributes: (string * typ * bool * access) list;
...
type program = {
    ...
    globals: ((string * expr option) * typ) list;
    main: seq;
  }